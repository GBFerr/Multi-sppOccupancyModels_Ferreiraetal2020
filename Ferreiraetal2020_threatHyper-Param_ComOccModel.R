...
# model with 2 hyper-parameters according to threat (national); used for community-level inference only
...
# same data as in single hyper-param model plus variable assigining spp to 2 threat categories
###################################################################################################
...
summary(spindex)
Threat <- spindex[,4]
str(Threat)       # indicator of threated or non-threatned according to Brazilian redlist 2014

# Step 3 - Bundle and summarize data
# most of data objectives called above, in the 1st model

str(sp.data <- list(n = n, nzeroes = nzeroes, J = J, K = K, X = X,
                    a=Threat, # indicator of threat status (MMA 2014)
                    pa_type = pa_type, 
...
cat("
    ...
    for(z in 1:2){            ##  #Z loops over the 2 threat categories;
    ...
    psiMulti[i] ~ dnorm(mu.Multi[a[i]], tau.Multi[a[i]])
    psiStrict[i] ~ dnorm(mu.Strict[a[i]], tau.Strict[a[i]])
    alphaRoad[i] ~ dnorm(mu.alphaRoad[a[i]], tau.alphaRoad[a[i]])
    alphaWat[i] ~ dnorm(mu.alphaWat[a[i]], tau.alphaWat[a[i]])
    alphaNDVI[i] ~ dnorm(mu.alphaNDVI[a[i]], tau.alphaNDVI[a[i]])
    
    betaNtrail[i] ~ dnorm(mu.betaNtrail[a[i]], tau.betaNtrail[a[i]])
    betaYtrail[i] ~ dnorm(mu.betaYtrail[a[i]], tau.betaYtrail[a[i]])
    betaCT2[i] ~ dnorm(mu.betaCT2[a[i]], tau.betaCT2[a[i]])
    betaCT3[i] ~ dnorm(mu.betaCT3[a[i]], tau.betaCT3[a[i]])
    ...
    for(h in 1:2){
    Com.Threat.PAeffect[h] <- mu.Strict[h] - mu.Multi[h]
    }
...
outHypeThreat <- jags(sp.data, sp.inits, params1, "modelHypeThreat.txt", 
